 ...
 */
(function (exports) {
  var apply_patch, InvalidPatch, PatchApplyError, JSONPatch, JSONPointer,_operations;
   ...
   * Example (in browser)
   *    
   *     <script src="jsonpatch.js" type="application/javascript"></script>
   ...
   */
  exports.InvalidPatch = InvalidPatch = function InvalidPatch(message) {Error.call(this, message); this.message = message;};
  InvalidPatch.prototype = new Error();
  /* Public: Error thrown if the patch can not be apllied to the given document
   */  
  exports.PatchApplyError = PatchApplyError = function PatchApplyError(message) {Error.call(this, message); this.message = message;};
   ...
   * context.
   */
  JSONPointer.prototype._getSegment = function (index, doc) {
  ...
  JSONPointer.prototype.replace = function (doc, value) {
    if (0 === this.length) {
      // Points to the root, so we want to replace the whole doc with
      // the new value
      return value;
    }
  ...
  function makeOperation(pointer, method, value) {
    return function (doc) {
  ...
  JSONPatch.prototype._compile = function (patch) {
    ...
    for (i = 0; i < patch.length; i++) {
      ...
      this.compiledOps[i] = makeOperation(pointer, _operations[op], value);
    }
  };
